...
/*
  if (DEBUGDATARAW) {
    BlueTooth.println("GoBLE availalbe -> new data package!");
    for (int i = 0; i < rDataPack.commandLength; i++) {
      BlueTooth.print(bleQueue.pop(), HEX);
    }
    BlueTooth.println();
  }

  if (DEBUGPARSER) {
    BlueTooth.print("GoBLE availalbe -> bleQueue Counter: ");
    BlueTooth.print(bleQueue.count());
    BlueTooth.println();
  }
*/
...
int _GoBLE::bleDataPackageParser() {
  ...
  */
  ...
  if (DEBUGCHECKSUM) {
    BlueTooth.print("Parser -> digitalButtonLength: ");
    BlueTooth.println(digitalButtonLength);
  }
  ...
  if (DEBUGCHECKSUM) {
    BlueTooth.print("Parser -> sum calculation: ");
    BlueTooth.println(calculateSum);

    BlueTooth.print("Parser -> checkSum byte value: ");
    BlueTooth.println(rDataPack.checkSum);
  }
...
void  _GoBLE::bleDataReceiver() {
  ...
  if (DEBUGDATARECEIVER) {
    BlueTooth.print("bleDataReceiver -> new data:");
    BlueTooth.println(inputByte, HEX);
  }
    ...
    for (int i = 0; i < MAXBUTTONID; i++) {
      if (_button[i] == PRESSED) {
        if (DEBUGUPDATEBUTTON) {
          BlueTooth.print("updateButtonState -> clear Pressed button number: ");
          BlueTooth.println(i);
        }
        _button[i] = RELEASED;
      }
